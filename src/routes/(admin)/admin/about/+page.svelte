  ...
  <section class="bg-white p-6 rounded-2xl shadow">
    ...
    {#if editingHero}
      ...
      <form method="post" action="?/updateHero" class="space-y-4">
        ...
        <input type="text" name="bgImage" value={data.hero?.bgImage ?? ""} class="w-full p-2 border rounded" />

        <label class="block">Main Heading</label>
        <input type="text" name="mainHeading" value={data.hero?.mainHeading ?? ""} class="w-full p-2 border rounded" />

        <label class="block">Subheading</label>
        <textarea name="subheading" rows="2" class="w-full p-2 border rounded">{data.hero?.subheading ?? ""}</textarea>

        <div class="flex gap-3 mt-4">
          <button type="submit" class="px-5 py-2.5 bg-pink-600 text-white rounded-xl">Save</button>
          <button type="button" class="px-5 py-2.5 bg-gray-200 text-gray-700 rounded-xl" on:click={() => (editingHero = false)}>Cancel</button>
        </div>
      </form>
    {:else}
