<script lang="ts">
  ...
  let galleryFiles: Record<string, FileList | null> = {};
  let galleryFileNames: Record<string, string> = {};
  let galleryPreviewSrcs: Record<string, string | null> = {};

  $: {
    if (galleryFiles) {
      for (const key in galleryFiles) {
        const file: File | null = galleryFiles[key]?.[0] ?? null;
        if (file) {
          galleryFileNames[key] = file.name;
          const reader = new FileReader();
          reader.onload = () => {
            galleryPreviewSrcs[key] = reader.result as string;
            // Reassign to trigger Svelte reactivity
            galleryPreviewSrcs = galleryPreviewSrcs;
          };
          reader.readAsDataURL(file);
        } else {
          galleryFileNames[key] = "No file chosen";
          galleryPreviewSrcs[key] = null;
          // Reassign to trigger Svelte reactivity
          galleryPreviewSrcs = galleryPreviewSrcs;
        }
      }
...
<div class="space-y-10">
  ...
  <section class="bg-white p-6 rounded-2xl shadow">
    ...
    {#if editingGalleryImages}
    <form method="post" action="?/updateGalleryImages" enctype="multipart/form-data">
      ...
      {#each draft.galleryImages as img, i}
        ...
        <div class="mb-4 border-b pb-4">
          ...
          <input
            id={`galleryImage_${i}`}
            type="file"
            name={`galleryImage_${i}`}
            accept="image/*"
            class="hidden"
            bind:files={galleryFiles[`${i}`]}
          />
          ...
          <label
            ...
            class="inline-block cursor-pointer px-4 py-2 bg-pink-400 text-white rounded-2xl hover:bg-pink-600 transition"
          >Choose File</label>
          <span class="ml-2 text-gray-600 italic">{galleryFileNames[`${i}`] || "No file chosen"}</span>

          {#if galleryPreviewSrcs[`${i}`]}
            <img src={galleryPreviewSrcs[`${i}`]} alt="Preview" class="h-32 object-cover rounded mt-2" />
          {/if}
